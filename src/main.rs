...
async fn main() {
    ...
    match commands.subcommand() {
        Some(("oauth", sub_match)) => {
            let client_id = sub_match.get_one::<String>("Client_ID").expect("Expected Client Id");
            let client_secret = sub_match.get_one::<String>("Client_Secret").expect("Expected Client Secret");
            ...
            let authinfo: CustomAuthData = auth.launch(*bedrock, &client_secret).await.expect("Failed to launch");

            println!("Bearer: {:?}, \n UUID: {:?}, \n Expire_in: {:?}, \n XtsToken: {:?}", authinfo.access_token, authinfo.uuid, authinfo.uuid, authinfo.xts_token);
        },
        Some(("devicecode", sub_match)) => {
            ...
            let auth = auth_future.await.expect("Failed to create DeviceCode instance");
